...
use crate::{crypto::{calculate_mac, decrypt_int, encrypt_int, verify_mac_int}, libsignal::constants::{Chain, ChainKey, ChainType, KeyPair, SessionEntry}, utils::{derive_secrets_int, generate_key_pair_int, shared_secret_int}};
use prost::Message;
...
impl SessionCipher {
  ...
  pub fn encrypt(
    ...
    let keys = derive_secrets_int(&message_key.encode_to_vec(), &[0u8; 32], WHISPER_MESSAGE_KEYS, 3);
    ...
    let whisper_msg = WhisperMessage {
      ...
      ciphertext: Some(encrypt_int(keys[0].as_ref(), data, &keys[2][..16]).map_err(|e| format!("Encryption failed: {:?}", e))?),
    };

    let msg_buf = Self::encode_whisper_message(&whisper_msg)?;
    ...
    let mac = calculate_mac(&keys[1], &mc)
        .map_err(|_| "Failed to calculate MAC")?;
    ...
    let (message_type, body) = if let Some(pending) = &session.pending_pre_key {
      ...
      let encoded = Self::encode_prekey_message(&prekey_msg)?;
  ...
  pub fn decrypt_whisper_message(
    ...
    let message = Self::decode_whisper_message(message_proto)?;

    // ratchet
    let ephemeral_key = message.ephemeral_key.as_ref().ok_or("Missing ephemeral key")?;
    let ephemeral_key_array: &[u8; 33] = ephemeral_key.as_slice().try_into()
        .map_err(|_| "Invalid ephemeral key length")?;
    self.maybe_step_ratchet(session, ephemeral_key_array, message.previous_counter.unwrap() as i32)?;
    ...
    verify_mac_int(&mc, &keys[1], mac_slice, 8);
    ...
    Ok(plaintext)
      ...
      fn fill_message_keys(chain: &mut Chain, counter: i32) -> Result<(), Box<dyn StdError>> {
        ...
        while next_counter < counter as i32 {
            ...
            let message_key = calculate_mac(&key, &[1u8])
            .map_err(|_| "Failed to derive message key")?;
            key = calculate_mac(&key, &[2u8])
            .map_err(|_| "Failed to derive next chain key")?;
            
            chain.message_keys.insert(next_counter, String::from_utf8_lossy(&message_key).to_string());
        }
    ...
    fn encode_whisper_message(msg: &WhisperMessage) -> Result<Vec<u8>, Box<dyn StdError>> {
        let mut buf = Vec::new();
    ...
    fn decode_whisper_message(data: &[u8]) -> Result<WhisperMessage, Box<dyn StdError>> {
        let msg = WhisperMessage::decode(data)?;
        Ok(msg)
    }

    fn encode_prekey_message(msg: &PreKeyWhisperMessage) -> Result<Vec<u8>, Box<dyn StdError>> {
        let mut buf = Vec::new();
        buf.reserve(msg.encoded_len());
        msg.encode(&mut buf)?;
        Ok(buf)
    }
    fn decode_prekey_message(data: &[u8]) -> Result<PreKeyWhisperMessage, Box<dyn StdError>> {
