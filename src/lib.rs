...
pub fn key_pair<'a>(env: Env, priv_key: Buffer) -> Result<Object<'a>> {
    ...
    let priv_key_array: [u8; 32] = priv_key.as_ref().try_into()
...
pub fn verify(sig: Buffer, pub_key: Buffer, message: Buffer) -> Result<bool> {
    ...
    let sig_array: &[u8; 64] = sig.as_ref().try_into()
        .map_err(|_| Error::from_reason("Invalid signature"))?;
    
    verify_int(pub_key.as_ref(), message.as_ref(), sig_array).map_err(|e| Error::from_reason(format!("Verification failed: {}", e)))
